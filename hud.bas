...
sub hud_Print cdecl (X as GLint,Y as  GLint,W as integer,H as integer,Scale as integer, Vis as double ,  _String as zstring ptr, ...)
	...
	dim as integer _Width = 0 

	memset(@LineText(0,0), &H00, ArraySize(LineText))
	...
	do while i < lines
	j = 0
		do while j < cast(integer,strlen(cast(zstring ptr,@linetext(i,0))))
		       if (LineText(i,j) = asc(!"\t") ) then LineText(i,j)  = &H7F
			if (LineText(i,j)  < &H80) then  LineWidths(i) += zHUD.CharWidths(LineText(i,j)- 32)
	j +=1
	loop
	if(LineWidths(i) > _Width) then _Width = LineWidths(i)
	i += 1 
	loop
...
sub hud_Print2 cdecl (X as GLint,Y as  GLint,W as integer,H as integer,Scale as integer, Vis as double ,  _String as zstring ptr, ...)
