	...
	select case (Character) 
		...
		case asc(!"\n")
			...
			' MSK_ConsolePrint(MSK_COLORTYPE_OKAY, !"%i\n",OnObject)
				...
				ExitDlg = TRUE
				if(MSK_UI_Dialog_ObjButton_IsOkay(@_Object(OnObject))) then IsOkay = TRUE
			end if
	...
	do while i < arraysize(_object)
		...
		'	MSK_ConsolePrint(0, "IN NUM") 
			...
			case MSK_UI_DLGOBJ_LINE 
				      dim as int32_t _Length = 0 ' temporary fix
					sscanf(_Object(i).ObjParameters, !"%i", @_Length)
					if _Length = -1 then 
					 _Length = Dlg->w - 4
					end if
					
					mvwhline(Dlg->Win, _Object(i).y + 1, _Object(i).x + 1, ACS_HLINE, _Length)
...
sub MSK_UI_Dialog_SyncSelection(Dlg as __MSK_UI_Dialog_Data ptr )
	...
	do while i < ArraySize(_Object) 
		if(_Object(i).ParentHandle = Dlg->Handle) then
			if(_Object(i).Order = Dlg->ObjSelected(1)) then
				Dlg->ObjSelected(0) = i
				exit do
			end if
		end if
