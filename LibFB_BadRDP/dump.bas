...
Sub RDP_Dump_DumpTriangle (Vtx As __Vertex Ptr, VtxID As integer Ptr)
	...
	for i as integer = 0 to 3 - 1
		...
		vtT = -Vtx[VtxID[i]].RealT0
		 ...
		 if IsInfinite(vtS) orelse __isnan(vtS) then vtS = 0.0f
		 if IsInfinite(vtT) orelse __isnan(vtT) then vtT = 0.0f
		if(Texture(0).CMS = G_TX_MIRROR) then vtS = vtS/2
		if(Texture(0).CMT = G_TX_MIRROR) then vtT = vtT/2
		if(Texture(1).CMS = G_TX_MIRROR) then vtS2 = vtS2/2
		if(Texture(1).CMT = G_TX_MIRROR) then vtT2 = vtT2/2		
...
sub  RDP_Dump_SelectMaterial(MatID as integer)

 
	 If _System.FileWavefrontObj = NULL Orelse _System.FileWavefrontMtl = NULL Orelse _System.ObjDumpingEnabled = NULL Then Return  
 

	fprintf(_System.FileWavefrontObj, !"usemtl material_%d\n", MatID) 
